...
/** Holds a move-to-trash action waiting on the user's Delete/Cancel confirmation. */
private class PendingTrashConfirm(val count: Int, val onConfirm: () -> Unit)
...
private fun GalleryContent(
    ...
    // Delete/Cancel confirmation shown before anything is moved to the trash bin.
    var pendingTrashConfirm by remember { mutableStateOf<PendingTrashConfirm?>(null) }
    ...
    when (val r = route) {
        is Route.Viewer -> {
            ...
            PhotoViewerScreen(
                ...
                onDelete = { item ->
                    pendingTrashConfirm = PendingTrashConfirm(count = 1) {
        ...
        is Route.AlbumDetail -> {
            ...
            AlbumDetailScreen(
                ...
                onDeleteSelected = {
                    pendingTrashConfirm = PendingTrashConfirm(count = selectedIds.size) {
        ...
        is Route.TrashBin -> {
            val trashItems = remember(allMedia, trashedIds) { viewModel.trashedItems() }
            TrashBinScreen(
                items = trashItems,
                onBack = { route = Route.Main },
    ...
    pendingTrashConfirm?.let { pending ->
        AlertDialog(
            onDismissRequest = { pendingTrashConfirm = null },
            title = {
                Text(if (pending.count == 1) "Delete item?" else "Delete ${pending.count} items?")
            },
            text = {
                Text(
                    if (pending.count == 1)
                        "This item will be moved to the trash bin. You can restore it within 30 days."
                    else
                        "These items will be moved to the trash bin. You can restore them within 30 days."
                )
            },
            confirmButton = {
                TextButton(onClick = {
                    pending.onConfirm()
                    pendingTrashConfirm = null
                }) { Text("Delete") }
            },
            dismissButton = {
                TextButton(onClick = { pendingTrashConfirm = null }) { Text("Cancel") }
            }
        )
    }
}
