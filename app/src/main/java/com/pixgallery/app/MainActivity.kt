...
import com.pixgallery.app.ads.RewardedAdManager
...
import com.pixgallery.app.ui.components.BannerAdView
...
import com.pixgallery.app.ui.screens.DuplicateFinderScreen
import com.pixgallery.app.ui.screens.MemoriesScreen
...
import com.pixgallery.app.ui.screens.StorageAnalyzerScreen
import com.pixgallery.app.ui.screens.TrashBinScreen
import com.pixgallery.app.ui.screens.VaultScreen
...
private sealed class Route {
    ...
    data object DuplicateFinder : Route()
    data object Memories : Route()
    data object StorageAnalyzer : Route()
}
...
private fun GalleryContent(
    ...
    // Shows a full-screen video ad every 3rd photo opened (see everyNActions
    // in RewardedAdManager). useTestAd = false so the real ad unit ID is used
    // and impressions/clicks actually count and earn revenue - this app is
    // being published, so it must not ship with test ads.
    val rewardedAdManager = remember {
        RewardedAdManager(context, useTestAd = false, everyNActions = 3)
    }
    ...
    when (val r = route) {
        is Route.Viewer -> {
            val viewerItems = when (val source = r.source) {
                ViewerSource.AllMedia -> allMedia.filter { it.id !in trashedIds && it.id !in hiddenIds }
                is ViewerSource.Album -> allMedia.filter { it.bucketId == source.bucketId && it.id !in trashedIds && it.id !in hiddenIds }
                ViewerSource.Favorites -> allMedia.filter { it.id in favoriteIds && it.id !in trashedIds && it.id !in hiddenIds }
            }
            PhotoViewerScreen(
                items = viewerItems,
        ...
        is Route.AlbumDetail -> {
            ...
            val albumItems = if (isFavoritesAlbum) {
                allMedia.filter { it.id in favoriteIds && it.id !in trashedIds && it.id !in hiddenIds }
            } else {
                allMedia.filter { it.bucketId == r.bucketId && it.id !in trashedIds && it.id !in hiddenIds }
            }
            AlbumDetailScreen(
                ...
                onItemClick = { item ->
                    if (selectionMode) {
                        ...
                        rewardedAdManager.maybeShow(context as Activity) {
                ...
                onHideSelected = {
                    viewModel.hideItems(selectedIds)
                    selectionMode = false
        ...
        Route.Vault -> {
            VaultScreen(
                hiddenItems = allMedia.filter { it.id in hiddenIds },
                isPinSet = viewModel.isVaultPinSet(),
                onCreatePin = { pin -> viewModel.setVaultPin(pin) },
                onCheckPin = { pin -> viewModel.checkVaultPin(pin) },
                onUnhide = { item -> viewModel.unhideItem(item.id) },
                onDeleteForever = { item -> viewModel.permanentlyDelete(setOf(item.id)) },
                onBack = { route = Route.Main }
            )
        }
        ...
        Route.DuplicateFinder -> {
            DuplicateFinderScreen(
                allMedia = allMedia.filter { it.id !in trashedIds && it.id !in hiddenIds },
                onBack = { route = Route.Main },
                onMoveToTrash = { ids ->
                    ids.forEach { viewModel.moveToTrash(it) }
                    Toast.makeText(context, "Moved ${ids.size} item(s) to bin", Toast.LENGTH_SHORT).show()
                }
            )
        }

        Route.StorageAnalyzer -> {
        ...
        Route.Memories -> {
            MemoriesScreen(
                memories = viewModel.onThisDayMemories(),
                onItemClick = { item ->
                    val memoriesVisible = allMedia.filter { it.id !in trashedIds && it.id !in hiddenIds }
                    val idx = memoriesVisible.indexOf(item)
                    if (idx >= 0) {
                        rewardedAdManager.maybeShow(context as Activity) {
        ...
        Route.Main -> {
            Scaffold(
                    ...
                    if (!selectionMode) {
                        ...
                        androidx.compose.material3.Surface(tonalElevation = 3.dp) {
                            SelectionActionBar(
                                ...
                                onSelectAll = {
                                    viewModel.selectAll(allMedia.filter { it.id !in trashedIds && it.id !in hiddenIds }.map { it.id })
                                },
                                onHide = {
                                    viewModel.hideItems(selectedIds)
                                    selectionMode = false
                ...
                val visibleMedia = allMedia.filter { it.id !in trashedIds && it.id !in hiddenIds }
                when (selectedTab) {
                    GalleryTab.PHOTOS -> {
                        PhotosScreen(
                    ...
                    GalleryTab.RECOMMENDED -> {
                        RecommendedScreen(
                            onTrashBin = { route = Route.TrashBin },
                            onVault = { route = Route.Vault },
                            onDuplicateFinder = { route = Route.DuplicateFinder },
                            onMemories = { route = Route.Memories },
                            onStorageAnalyzer = { route = Route.StorageAnalyzer },
                            modifier = Modifier.padding(padding)
                        )
                    }
                }
