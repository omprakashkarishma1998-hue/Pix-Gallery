...
class AppOpenAdManager(
    ...
    // Never on the very first cold start - an ad the instant someone opens a
    // freshly installed app is a bad first impression and reads as an app
    // whose only purpose is showing ads. Skipped once, then normal cooldown
    // applies for every later foreground.
    private var isFirstForeground = true
    private var lastShownAtMillis: Long = 0L
    // Minimum gap between App Open ads. Showing one on *every* single
    // foreground (e.g. quickly switching to another app to copy something,
    // then back) is intrusive - this caps it to at most once per this window.
    private val minIntervalMillis = 15 * 60 * 1000L
    ...
    private fun showAdIfAvailable() {
        ...
        if (now - lastShownAtMillis < minIntervalMillis) return
